...
- **Sentence-level streaming reverted in extension Go Live** — caused speaker confusion in noisy environments and 8s+ stale-audio drains when speakers talk faster than playback. Replaced with **turn-based**: single global lock, whole-utterance pipeline, lock held through TTS playback. Predictable ~2.5s end-of-speech-to-first-audio. `SarvamSentenceStreamer` class kept as dead code for future use.
- **Aggressive first-chunk TTS split** (`splitForFastFirstChunk` in extension `pipeline.js`): when translated text exceeds ~90 chars, peel off the first sentence (or clause if no sentence break) and TTS it in parallel with the remainder. First audio plays as soon as the first chunk completes — for multi-sentence translations this saves ~600-1000ms before the listener hears anything.
...
- **Webapp `textToSpeech` accepts `onChunk`** (matches extension). `englishToSpeech` now pipelines Bulbul chunks into playback so chunk 1 plays while chunks 2+ are still being generated — roughly halves perceived first-audio latency for multi-chunk hold-to-speak turns.
