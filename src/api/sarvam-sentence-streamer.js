/**
 * sarvam-sentence-streamer.js — wraps SarvamStreamingSTT with sentence-level
 * emission. Fires translate+TTS per sentence as partials land, so the listener
 * hears the first translated sentence while the speaker is still producing
 * the second. Biggest perceived latency win without changing TTS providers.
 *
...
export class SarvamSentenceStreamer {
  constructor({ apiKey, languageCode, mode, onSentence, onPartial }) {
    this.onSentence = onSentence;
    this.onPartial  = onPartial;
    this._stream = new SarvamStreamingSTT({
      apiKey, languageCode, mode,
  ...
  _emitFinishedSentences() {
    ...
    while (this._settled < text.length) {
      ...
      if (!m) break;
      const endAbs = this._settled + m.index + 1;
